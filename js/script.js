...
var ctx=canvas.getContext('2d'),cur=-1,TOTAL=122,PATH='https://cdn.jsdelivr.net/gh/ale-naslim/voy-lp-de-onde-vem-a-caneta@main/frames/pens/frame_',EXT=window.__webp?'webp':'jpg';
...
function draw(k){var img=imgAt(k);if(!img)return;var dpr=__dpr(),cw=canvas.width/dpr,ch=canvas.height/dpr,iw=img.naturalWidth,ih=img.naturalHeight,scale=Math.min(cw/iw,ch/ih),dw=iw*scale,dh=ih*scale,dx=(cw-dw)/2,dy=(ch-dh)/2;ctx.clearRect(0,0,cw,ch);ctx.drawImage(img,dx,dy,dw,dh);cur=k;}
...
mountScrub(false);
mountContainScrub('boxScrub','https://cdn.jsdelivr.net/gh/ale-naslim/voy-lp-de-onde-vem-a-caneta@main/frames/caixa/frame_',121,false);
